...
export default function ClubDetailClient({ clubId }: { clubId: number }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="p-6 bg-white border-b border-gray-100 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        ...
        <div className="w-10 h-10 rounded-full bg-yellow-100 flex items-center justify-center">{logo}</div>
      </header>
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="flex items-start gap-5">
              <div className="w-16 h-16 rounded-2xl bg-yellow-100 flex items-center justify-center text-3xl">{logo}</div>
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  ...
                  {derivedCoreBadges.map((t) => (
                    <span
                      key={t}
                      className="px-3 py-1 bg-gray-50 text-gray-700 rounded-full text-xs font-semibold border border-gray-100"
                    >
                      {t}
                    </span>
                  ))}
                  {club.tags.slice(0, 3).map((t) => (
                    <span
                      key={t}
                      className="px-3 py-1 bg-white text-gray-500 rounded-full text-xs font-semibold border border-gray-100"
                    >
                      {t}
                    </span>
                  ))}
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <div className="text-xs text-gray-400">当前已有 {club.memberCount} 人关注/报名</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              ...
              {myApplication && <span className="text-xs text-gray-400">我的状态：{statusText.label}</span>}
            </div>
            <ul className="mt-6 space-y-3">
              {club.requirements.map((r) => (
                <li key={r} className="text-gray-700 font-semibold">
                  • {r}
                </li>
              ))}
            </ul>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                ...
                <ul className="mt-4 space-y-3">
                  {benefits.map((b) => (
                    <li key={b} className="text-gray-700 font-semibold">
                      • {b}
                    </li>
                  ))}
                </ul>
              ...
              <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                ...
                <ul className="mt-4 space-y-3">
                  {memberProfile.map((p) => (
                    <li key={p} className="text-gray-700 font-semibold">
                      • {p}
                    </li>
                  ))}
                </ul>
                  ...
                  >
                    {h.src ? (
                      h.type === 'video' ? (
                        ...
                        <img src={h.src} alt={h.title} className="absolute inset-0 w-full h-full object-cover" />
                      )
                    ) : (
                    ...
                    <div className="absolute inset-0 p-4 flex flex-col justify-between">
                      ...
                      <div className="flex items-end justify-between">
                        ...
                        {h.type === 'video' && (
                          <div className="w-10 h-10 rounded-full bg-black/70 text-white flex items-center justify-center font-bold">
                            ▶
                          </div>
                        )}
                      </div>
          ...
          >
            ...
            >
              ...
              <div className="mt-3 text-gray-500 leading-relaxed">
                你将把基础档案（姓名、学院、年级、学号、个人简介等）发送给社团管理员用于筛选与联系。
              </div>
              <div className="mt-6 rounded-2xl bg-gray-50 border border-gray-100 p-5">
                ...
                <div className="mt-2 text-sm text-gray-600">
                  年级：{currentStudent.grade} · 学号：{currentStudent.studentNo}
                </div>
          ...
          >
            ...
            >
              ...
              <p className="text-gray-500">
                报名成功，等待社团确认！请务必扫码加入官方交流群，后续面试安排及社费缴纳事宜将在群内通知。
              </p>
